...
void* BENSCHILLIBOWLCook(void* tid) {
		...
		while(1) {
			...
			printf("Cook #%d fulfilled order #%d: %s for customer number %d\n",
...
int main() {
		...
		for (int i = 0; i<NUM_CUSTOMERS; i++) {
			if (pthread_create(&customers[i], NULL, BENSCHILLIBOWLCustomer, (void *)(long)i) != 0) {
				perror("Failed to create custmer thread");
				exit(EXIT_FAILURE);
			}
		}

		for (int i = 0; i<NUM_COOKS; i++) {
			if (pthread_create(&customers[i], NULL, BENSCHILLIBOWLCook, (void *)(long)i) != 0) {

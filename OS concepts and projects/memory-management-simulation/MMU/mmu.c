...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    found:
        ...
        // If block is larger than requested, create a fragment
        if((blk->end - blk->start + 1) > blocksize) {
            ...
            // Add fragment back to free list according to policy
            if(policy == 1) {
        ...
        // Update block PID and add to allocated list
        blk->pid = pid;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    // Check if block exists in allocated list
    if(!list_is_in_by_pid(alloclist, pid)) {
        printf("Error: Can't locate Memory Used by PID: %d\n", pid);
        return;
    }
    
    // Get block index and remove from allocated list
    ...
    // Add block back to free list according to policy
    if(policy == 1) {
